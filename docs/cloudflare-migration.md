...
- Convert `workdoe/schema.sql` into D1 migrations.
- Replace local upload writes with Workdoe Worker upload routes that write to R2 using scoped keys:
  - `jobs/{job_id}/{uuid}.{ext}`
  - `contractors/{contractor_id}/{uuid}.{ext}`
...
- Enqueue `workdoe-media-review` after each successful R2 upload and D1 metadata insert so moderation has an audit trail before future image-safety checks are added.
...
- `GET /`, `GET /login`, and `GET /start`, which serve a same-domain Clerk email-code entry shell with the Workdoe deer logo, live lead map/list, and no hosted-login redirect.
...
- `GET /api/jobs/open`, which reads open jobs from D1 for the login/start map and returns only public lead-card fields with approximate pins.
...
- `POST /api/jobs`, which lets a Clerk-linked client create a D1 job after server-side Turnstile validation, curated category/DMV ZIP validation, and approximate coordinate derivation; the `/jobs/new` shell can then upload selected photos to the created job through the private media route.
...
- `/api/admin/users/:id/:action`, `/api/admin/jobs/:id/:action`, `/api/admin/photos/job/:id/:action`, `/api/admin/photos/contractor/:id/:action`, `/api/admin/messages/:id/hide`, and `/api/admin/reports/:id/resolve`, which let active Clerk-linked admins perform the local moderation actions with D1 audit rows.
...
- Email queue consumption through Cloudflare Email Service's `EMAIL` binding for supported Workdoe transactional messages, including fallback login codes, fallback password resets, stale mini-bid reminders, and moderation digests.
...
The Cloudflare same-domain entry shell intentionally mirrors the local `/start` and `/login` direction: the lead map and lead list appear before account creation, Clerk mounts inside `workdoe.com`, email-code sign-in stays on the same site, and the shell only renders public job facts. `/login` is kept to sign-in only; after Clerk verification it checks the Workdoe session bridge and sends unlinked identities to `/start` with the selected job preserved so role/profile creation stays explicit.
...
The Cloudflare contractor profile API intentionally mirrors the local profile form contract: only active contractor accounts can update their own profile, curated trade choices preserve Workdoe ordering, intro/website/year limits match the local app, and profile updates are audited.
...
- Write directly through the `MEDIA` R2 binding, never the Cloudflare REST API.
...
- Set `WORKDOE_TURNSTILE_SITE_KEY` and `WORKDOE_TURNSTILE_SECRET_KEY` to enable Turnstile on start, login, job posting, mini-bid, and report forms.
- Keep `WORKDOE_CLERK_LOGIN_MODE=same_domain_email_code`; the app rejects redirect-style Clerk modes.
- `/login` and `/start` mount Clerk in-page when Clerk mode is enabled; they do not send users to hosted Clerk login pages.
...
Cloudflare Email Sending must be onboarded for `workdoe.com` before production email delivery. The Worker uses a restricted `EMAIL` binding that can send only from `no-reply@workdoe.com`. The current queue consumer supports fallback login codes, fallback password resets, client stale-match reminders, and the admin moderation digest. Password reset links must stay on `https://workdoe.com` or `https://www.workdoe.com`; invalid email payloads are audited and acknowledged, while transient send failures are audited and retried by Cloudflare Queues.

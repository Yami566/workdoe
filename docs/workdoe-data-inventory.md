...
- Treat contractor insurance, license, experience, and profile statements as
  self-reported unless a later verification process is approved.
...
| Project template | Owner-defined name plus service family/task, category, title, description, coarse project setting, optional budget, source-project ID, timestamps | Reuse recurring scope without silently carrying stale or sensitive project state | D1 `client_project_templates` | Owning consumer and active administrators through controlled operations. The schema excludes city, state, ZIP, desired date, photos/media paths, bids, messages, close outcomes, and completion state; use opens the six-step composer with location/date blank |
...
| Pre-verification draft | Project fields above except email and photos; random token hash and expiry | Let a consumer begin before email verification | D1 `job_drafts`; HttpOnly draft cookie | Token holder until consumed or expired; not publicly listed; the coarse setting follows the same restrictions as the published project |
...
| Consumer bid comparison projection | Up to four pending bid IDs, contractor ID/display name/trades, price range, timeline, availability, received-order position, self-reported years/insurance presence, current source-checked credential count, and mutually verified Workdoe project count | Let the owning consumer compare like-for-like offer and provider facts before opening a profile or approving a match | Computed in shared local/Worker Python code from existing bid/profile/credential/completion records; not stored separately | Owning consumer and active administrators through the protected project view. The projection excludes email, phone, address, ZIP, coordinates, website, license/credential identifier, review narrative, private messages, fit score, recommendation, and paid placement. Source checks, completion history, and self-reported fields remain separately labeled and never become a composite score |
...
| Approved conversation | Thread participants, message body, sender, timestamp, hidden state | Private post-approval communication | D1 `threads` and `messages` | Approved consumer/contractor participants; administrators have read-only moderation inspection |
...
desired date, budget label, photo count, and approximate rounded pins. Workdoe's

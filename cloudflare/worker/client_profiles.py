...
DMV_STATES = {"DC", "MD", "VA"}
MAX_CLIENT_PROFILE_BODY_BYTES = 4096
MAX_SAVED_LOCATION_BODY_BYTES = 2048
...
def client_profile_payload(payload) -> dict[str, str]:
    if not isinstance(payload, dict):
        raise ClientProfileError(["Consumer profile payload must be a JSON object."])
...
def saved_location_payload(payload) -> dict[str, str]:
    if not isinstance(payload, dict):
        raise SavedLocationError(["Saved project area payload must be a JSON object."])
...
def saved_location_response(location) -> dict[str, str | int]:
    return {
        "id": int(row_value(location, "id", 0) or 0),
        "label": str(row_value(location, "label", "") or ""),
        "city": str(row_value(location, "city", "") or ""),
        "state": str(row_value(location, "state", "") or ""),
        "zip_code": str(row_value(location, "zip_code", "") or ""),
    }


def parse_saved_location_delete_path(path: str) -> int:
    prefix = "/api/client/locations/"
    suffix = "/delete"
    if not path.startswith(prefix) or not path.endswith(suffix):
        raise SavedLocationError(["Saved project area route is invalid."])

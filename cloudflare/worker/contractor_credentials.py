...
CREDENTIAL_STATUSES = {
    "self_reported", "pending", "verified", "expired", "rejected"
}
...
CREDENTIAL_REVIEW_ACTIONS = {
    "verify": "verified", "pending": "pending", "reject": "rejected", "expire": "expired"
}
...
CREDENTIAL_HOST_LABEL_RE = re.compile(r"[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?")
MAX_CONTRACTOR_CREDENTIAL_BODY_BYTES = 4096
CONTRACTOR_CREDENTIAL_REMOVE_RE = re.compile(
    r"^/api/contractor/credentials/([1-9][0-9]*)/remove/?$"
)
...
def parse_contractor_credential_remove_path(path: str) -> int:
    match = CONTRACTOR_CREDENTIAL_REMOVE_RE.match(path or "")
    if not match:
        raise ContractorCredentialError(["Unsupported credential claim route."])
...
def normalized_credential_source_url(value: str | None) -> str:
    ...
    if not raw_value or len(raw_value) > CREDENTIAL_SOURCE_MAX_LENGTH:
        return ""
    if any(character.isspace() or ord(character) < 32 or ord(character) == 127 for character in raw_value):
        return ""
    try:
        ...
        return ""
    if (
        parsed.scheme.lower() != "https" or not ascii_hostname or parsed.username
        or parsed.password or port not in {None, 443} or ascii_hostname == "localhost"
        or "." not in ascii_hostname
        or any(not CREDENTIAL_HOST_LABEL_RE.fullmatch(label) for label in ascii_hostname.split("."))
    ):
        return ""
    try:
        ...
        return ""
    return urlunparse(("https", ascii_hostname, parsed.path, parsed.params, parsed.query, parsed.fragment))
...
def contractor_credential_claim_payload(payload: dict) -> dict:
    ...
    if identifier_length < CREDENTIAL_IDENTIFIER_MIN_LENGTH:
        ...
        errors.append(f"Keep the credential identifier under {CREDENTIAL_IDENTIFIER_MAX_LENGTH} characters.")
    if len(claim["claimed_name"]) > CREDENTIAL_NAME_MAX_LENGTH:
        errors.append(f"Keep the name on the record under {CREDENTIAL_NAME_MAX_LENGTH} characters.")
...
def contractor_credential_review_payload(payload: dict, action: str, existing_source_url: str = "") -> dict:
    ...
    if len(review_note) > CREDENTIAL_REVIEW_NOTE_MAX_LENGTH:
        errors.append(f"Keep the review note under {CREDENTIAL_REVIEW_NOTE_MAX_LENGTH} characters.")
    if action in {"pending", "reject"} and not review_note:
        errors.append("Add a short review note for the contractor.")
    if errors:
        raise ContractorCredentialError(errors)
    return {"status": CREDENTIAL_REVIEW_ACTIONS[action], "source_url": source_url, "expires_at": expires_at or None, "review_note": review_note}


def credential_is_current(credential, today: str | None = None) -> bool:
    if credential is None:
        return False
    status = credential.get("status") if isinstance(credential, dict) else getattr(credential, "status", "")
    expires_at = credential.get("expires_at") if isinstance(credential, dict) else getattr(credential, "expires_at", "")
    return status == "verified" and (not expires_at or str(expires_at) >= (today or date.today().isoformat()))
...
def credential_response(credential, *, include_private: bool = False) -> dict:
    def value(key, default=""):
        ...
        return getattr(credential, key, default)
    ...
    result = {
        ...
        "credential_type_label": CREDENTIAL_TYPE_LABELS.get(credential_type, "Credential"),
        "jurisdiction": jurisdiction,
        "jurisdiction_label": CREDENTIAL_JURISDICTION_LABELS.get(jurisdiction, jurisdiction or "Other"),
        "status": status,
...
def public_credential_responses(credentials) -> list[dict]:
    return [credential_response(credential) for credential in credentials if credential_is_current(credential)]
